...
Objective: Create GetNewPoints function
...
class VehicleGeometry():
	...
	def getNewPoints(self, x, y, z, yaw, pitch, roll):
		...
		dcmM = Rotations.euler2DCM(yaw,pitch,roll)					#getting the DCM values from euler angles
		newPoints = MatrixMath.matrixMultiply(self.vertices, dcmM)	#Multiply DCM matrix with the self.verticies
		newPoints = MatrixMath.matrixOffset(newPoints,x,y,z)		#Offset to convert to a colum matrix
		newPoints = Rotations.ned2enu(newPoints)					#convert to ENU points
