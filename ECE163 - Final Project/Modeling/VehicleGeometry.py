...
class VehicleGeometry():
	...
	def getNewPoints(self, x, y, z, yaw, pitch, roll):
		...
		"""
		myPoints = self.vertices
		dcm = Rotations.euler2DCM(yaw, pitch, roll) # get rotation matrix from euler angles
		newPoints = MatrixMath.matrixMultiply(myPoints, dcm)  # convert inertial to body frame
		for i in newPoints: # update coordinates to reflect displacement
			i[0] += x
		for i in newPoints:
			i[1] += y
		for i in newPoints:
			i[2] += z
		newPoints = Rotations.ned2enu(newPoints) # change NED to ENU
		return newPoints
