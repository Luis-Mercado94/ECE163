...
class vehicleDisplay(QWidget):
	...
	def __init__(self, parent=None):
		...
		self.openGLWindow.setCameraPosition(distance=50, elevation=defaultElevation, azimuth=defaultAzimuth)
		...
		self.vehicleDrawInstance2 = VehicleGeometry.VehicleGeometry()

		# we need to grab the vertices for the vehicle each update
		rawPoints2 = [[y * metersToPixelRatio for y in x] for x in self.vehicleDrawInstance2.getNewPoints(99.2, 0, -100, 0, 0, 0)]
		newVertices2 = numpy.array(rawPoints2)

		# faces and colors only need to be done once
		newFaces2 = numpy.array(self.vehicleDrawInstance2.faces)
		newColors2 = numpy.array(self.vehicleDrawInstance2.colors)

		# we convert the vertices to meshdata which allows us not to have to translate points every time
		self.vehicleMeshData2 = pyqtgraph.opengl.MeshData(vertexes=newVertices2, faces=newFaces2, faceColors=newColors2)

		# and we create the meshItem, we do not smooth to make the triangles be clean colors
		self.openGLVehicle2 = pyqtgraph.opengl.GLMeshItem(meshdata=self.vehicleMeshData2, drawEdges=True, smooth=False,
														 computeNormals=False)

		# always add the vehicle to the display
		self.openGLWindow.addItem(self.openGLVehicle2)
